...
select ref_prod 
from produit 
where nom_prod = 'bottes' and couleur = 'rouge';
...
from produit natural join provenance natural join usine 
where ref_usine = 302;
...
select * 
from produit 
where couleur is not null and poids is not null;
...
select * 
from produit 
where couleur is null or poids is null;
...
select * 
from produit 
where couleur is not null and poids is null 
or couleur is null and poids is not null;
...
---
select nom_mag, nom_usine 
from magasin, usine 
where magasin.ville = usine.ville;
...
select distinct nom_prod 
from produit 
natural join provenance 
natural join usine 
where couleur = 'rouge' and ville = 'Nantes';
...
---
select distinct nom_prod, nom_mag
from produit 
natural join provenance 
...
---
select distinct nom_prod, nom_mag 
from produit natural join provenance natural join magasin 
where couleur='bleu' or couleur='rouge';
...
where p1.ref_mag <> p2.ref_mag and p1.ref_prod = p2.ref_prod 
...
---
select distinct p1.ref_mag, p2.ref_mag 
from provenance as p1, provenance as p2 
where p1.ref_mag <> p2.ref_mag and p1.ref_prod = p2.ref_prod 
and p1.ref_usine = p2.ref_usine limit 3;
...
select nom_prod 
from produit natural join provenance natural join usine 
where nom_usine = 'peugeot' and nom_prod like 'lampe%'
intersect
select nom_prod 
from produit natural join provenance natural join magasin 
where ville = 'Bordeaux' and nom_prod like 'lampe%';
...
---
select distinct ref_prod, couleur 
from produit natural join provenance as propro 
natural join usine as proprousi 
natural join magasin;
...
---
--- TODO avoid having the right couples and their inverses.

select distinct p1.nom_prod as pdt1, p2.nom_prod as pdt2
from (produit natural join provenance) as p1,
(produit natural join provenance) as p2
where p1.nom_prod <> p2.nom_prod
and (p1.poids between p2.poids - 2 and p2.poids + 2
or p2.poids between p1.poids - 2 and p1.poids + 2)
and (p1.ref_mag = p2.ref_mag);
...
--- TODO avoid having the right couples and their inverses.
select distinct p1.nom_prod as pdt1, p2.nom_prod as pdt2
from (produit natural join provenance natural join magasin) as p1,
(produit natural join provenance natural join magasin) as p2
where p1.nom_prod <> p2.nom_prod
and (p1.poids between p2.poids - 2 and p2.poids + 2
or p2.poids between p1.poids - 2 and p1.poids + 2)
...
--- TOCHECK because there is only 4 results in the table. We want 9 results.
select nom_prod, nom_mag 
from produit natural join provenance natural join magasin
where (couleur='rouge' or couleur='bleu') and quantite = 400;
